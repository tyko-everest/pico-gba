...
parameter:
                Bmp_ScanDir :   Displays the LCD scanning mode of the BMP
picture Lcd_ScanDir :   LCD normal display scan
********************************************************************************/
void LCD_Show_bmp(LCD_SCAN_DIR Bmp_ScanDir, LCD_SCAN_DIR Lcd_ScanDir) {
    ...
    if ((res != FR_OK)) {
        if (res == FR_NO_FILESYSTEM) {
            ...
            GUI_DisString_EN(0, 32, "SD_CARD_NOT_FORMATTED", &Font24,
                             LCD_BACKGROUND, BLUE);
        } else {
            /* Display message: Fail to open directory */
            GUI_DisString_EN(0, 48, "SD_CARD_OPEN_FAIL", &Font24,
                             LCD_BACKGROUND, BLUE);
        }
    } else {
        // printf("file open \r\n");
    ...
    for (showtime = 0; showtime < filesnumbers; showtime++) {
        ...
        if (checkstatus == 0) {
            ...
            GUI_DisString_EN(0, 64, "SD_CARD_NOT_FOUND", &Font24,
                             LCD_BACKGROUND, BLUE);
        } else {
            /* Display message: File not supported */
            // Restore the default scan
            LCD_SetGramScanWay(Lcd_ScanDir);
            GUI_DisString_EN(0, 80, "SD_CARD_FILE_NOT_SUPPORTED", &Font24,
                             LCD_BACKGROUND, BLUE);
        }
