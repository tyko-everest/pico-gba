...
/** @defgroup STORAGE
 * @brief This file includes the Storage (FatFs) driver for the STM32 Nucleo
 * demo
 * @{
 */
...
#define PIXEL(__M)                                                             \
    ((((__M) + 31) >> 5) << 2) //����24λ���ɫ ÿһ�е����ؿ��ȱ�����4�ı���  ����0����
...
uint32_t Storage_OpenReadFile(uint8_t Xpoz, uint16_t Ypoz,
                              const char *BmpName) {
    uint16_t i, j, k, h;
    ...
    if (LCD_2_8 == id) {
        for (i = 0; i < height; i++) {
            ...
            for (j = 0; j < width; j++) {
                k = j * 3;
                pic[i * 240 + j] = (uint16_t)(((aBuffer[k + 2] >> 3) << 11) |
                                              ((aBuffer[k + 1] >> 2) << 5) |
                                              (aBuffer[k] >> 3));
            }
        }
        ...
        for (i = 0; i < height; i++) {
            ...
            for (j = 0; j < width; j++) {
                k = j * 3;
                pic[j] = (uint16_t)(((aBuffer[k + 2] >> 3) << 11) |
                                    ((aBuffer[k + 1] >> 2) << 5) |
                                    (aBuffer[k] >> 3));
                LCD_WriteData(pic[j]);
            }
    ...
    if (res == FR_OK) {
        ...
        for (;;) {
            ...
            if (!(MyFileInfo.fattrib & AM_DIR)) {
                ...
                if (j < MAX_BMP_FILES) {
                    if ((MyFileInfo.fname[i + 1] == 'B') &&
                        (MyFileInfo.fname[i + 2] == 'M') &&
                        (MyFileInfo.fname[i + 3] == 'P')) {
